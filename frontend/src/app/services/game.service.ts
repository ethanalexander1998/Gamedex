...
export class GameService {
  ...
  update(id: string, game: Partial<Game>): Observable<Game> {
    return this.http.put<Game>(`${this.apiUrl}/games/${id}`, game)
  }

  delete(id: string): Observable<{ message: string }> {
    return this.http.delete<{ message: string }>(`${this.apiUrl}/games/${id}`);
  }

  searchRawg(q: string): Observable<RawgResult[]> {
    return this.http.get<RawgResult[]>(`${this.apiUrl}/search`, { params: { q } });
  }
  ...
  generateRecommendations(): Observable<Recommendation[]> {
    return this.http.post<Recommendation[]>(`${this.apiUrl}/recommendations/generate`, {});
  }

  dismissRecommendation(id: string): Observable<Recommendation> {
    return this.http.put<Recommendation>(`${this.apiUrl}/recommendations/${id}/dismiss`, {});
  }
